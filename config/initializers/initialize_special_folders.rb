...
case RAILS_ENV.to_s
  ...
  when "staging"       then $domain = "http://gateway.com"
else
  raise "Unknown environnement : #{RAILS_ENV}"
end
...
case RAILS_ENV.to_s
  ...
  when "staging"       then $tmp_path = "public/tmp"
end
...
case RAILS_ENV.to_s
  ...
  when "staging"       then $base_executions_sources = "public/executions_sources"
else
  raise "Unknown environnement : #{RAILS_ENV}"
end
...
case RAILS_ENV.to_s
  ...
  when "staging"       then $default_client_folder = "no_client"
else
  raise "Unknown environnement : #{RAILS_ENV}"
end
...
case RAILS_ENV.to_s
  ...
  when "staging"       then $url_executions_sources = "executions_sources"
else
  raise "Unknown environnement : #{RAILS_ENV}"
end
